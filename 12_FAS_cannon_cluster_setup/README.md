...
The NFS file system on the Canon cluster is quite slow. To avoid time consuming setup, we opt to use singularity containers that are built-off docker container images as our standard dev environement.
...
This is going to print out `False` because the instance is not requested with GPU and singularity is missing the `--vn` option.
...
**Now launch the singularity container with the gpu binding**
